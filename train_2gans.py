...
# Training two repulsive BiGANs
parser.add_argument('--repulsive', action='store_false',
    help='Train two repulsive BiGANs')
parser.add_argument('--target_class', type=int,
    help='Target digit (0-9) of the MNIST dataset')
parser.add_argument('--outlier_prop', type=float, default=0.1,
    help='Proportion of outliers compared to inliers')
parser.add_argument('--repulsive_weight', type=float, default=1,
    help='Weight of the repulsive loss term in addition to BiGANs\' losses')
...
def get_batch(args, deploy=False):
    imb, ymb = dataset.train_provider.get_batch(args)
